...
/// emits `x86_64_v4` (AVX-512), `x86_64_v3` (AVX2+FMA) and a default (SSE2/NEON) variant; the caller
/// dispatches on the running CPU. Returns the pre-`finalize` reduction (e.g. the sum of squares for `L2`).
...
#define SUPPORTED_TYPES(action) \
    ...
    ColumnPtr executeWithTypes(const ColumnArray & array, size_t input_rows_count, const ColumnsWithTypeAndName & arguments) const
    {
        ...
        for (size_t row = 0; row < input_rows_count; ++row)
        {
            ...
            {
                /// SIMD-optimized path for same-type floating point: runtime-dispatched to the widest
                /// available ISA (AVX-512 / AVX2 / baseline). This is where `L2`/`L2Squared` pick up FMA.
                ResultType result;
                ...
                if (isArchSupported(TargetArch::x86_64_v4))
                    ...
                    result = normImpl_x86_64_v3<Kernel, ResultType>(data.data() + prev, array_size, kernel_params);
                else
#endif
